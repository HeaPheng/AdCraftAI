...
import { Loader2, KeyRound, Mail, ArrowRight, ArrowLeft } from "lucide-react"
...
function LoginForm() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!email || !password) {
      toast(t("auth_toast_login_fields_req"), "error")
      return
    }
    ...
    try {
      ...
      if (success) {
        toast(t("auth_toast_login_success"), "success")
        router.push("/dashboard")
      }
    } catch (err: unknown) {
      const errorMessage = err instanceof Error ? err.message : t("auth_toast_login_failed")
    ...
    <div className="relative flex min-h-screen items-center justify-center p-4 sm:p-6 lg:p-8 bg-background select-none">
      ...
      <div className="w-full max-w-[420px] space-y-6">
        ...
        <div className="flex flex-col items-center text-center mb-6">
          ...
          <p className="text-sm text-muted-foreground max-w-[300px] mt-1.5">
        ...
        <Card className="p-6 sm:p-8 border border-border/80 bg-card/60 backdrop-blur-md shadow-xl rounded-2xl relative overflow-hidden">
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between items-center">
                <label htmlFor="password" className="text-xs font-semibold text-foreground/80 tracking-wide">
          ...
          <div className="mt-5 p-3 rounded-xl bg-violet-500/[0.03] dark:bg-violet-500/[0.015] border border-violet-500/10 text-center">
            <span className="text-xs text-muted-foreground font-medium block mb-1.5">
              {t("auth_login_demo_credentials")}:
            </span>
            <button
              onClick={fillDemoCredentials}
              className="inline-flex items-center gap-1.5 text-xs text-violet-600 dark:text-violet-400 hover:underline font-semibold bg-violet-500/10 dark:bg-violet-500/5 px-2.5 py-1 rounded-full cursor-pointer transition-colors"
            >
              demo@adcraft.ai / password123
            </button>
          </div>
